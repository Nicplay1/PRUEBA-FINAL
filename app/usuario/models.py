...
class VehiculoResidente(models.Model):
    id_vehiculo_residente = models.AutoField(primary_key=True)
    placa = models.CharField(max_length=7, unique=True)
    tipo_vehiculo = models.CharField(max_length=10, choices=[('Carro', 'Carro'), ('Moto', 'Moto')])
    activo = models.BooleanField()
    documentos = models.BooleanField(default=False)
    cod_usuario = models.OneToOneField(Usuario, on_delete=models.CASCADE, db_column='cod_usuario')
...
class TipoArchivo(models.Model):
    ...
    tipo_documento = models.CharField(
        ...
        choices=[('SOAT','SOAT'),('Tarjeta de propiedad','Tarjeta de propiedad'),
                 ('Técnico-mecánica','Técnico-mecánica'),('Licencia','Licencia'),('Identidad','Identidad')]
    )
...
class ArchivoVehiculo(models.Model):
    ...
    ruta_archivo = models.FileField(upload_to='vehiculos/')
...
class Parqueadero(models.Model):
    id_parqueadero = models.IntegerField(primary_key=True)
...
class GanadorSorteo(models.Model):
    id_ganador = models.AutoField(primary_key=True)
    id_sorteo = models.ForeignKey(Sorteo, on_delete=models.CASCADE)
    id_detalle_residente = models.ForeignKey(DetalleResidente, on_delete=models.CASCADE)
...
class Visitante(models.Model):
    TIPO_VEHICULO_CHOICES = [('Carro', 'Carro'), ('Moto', 'Moto')]
    ...
    apartamento = models.IntegerField(null=True, blank=True)
...
class EntregaCorrespondencia(models.Model):
    ...
    id_usuario = models.ForeignKey(Usuario, on_delete=models.DO_NOTHING, related_name='entregas_usuario')
    id_correspondencia = models.ForeignKey(RegistroCorrespondencia, on_delete=models.DO_NOTHING, related_name='entregas_correspondencia')
    id_detalle_residente = models.ForeignKey(DetalleResidente, on_delete=models.DO_NOTHING, related_name='entregas_residente')
