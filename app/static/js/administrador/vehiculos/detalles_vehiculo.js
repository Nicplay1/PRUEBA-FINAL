...
// WebSocket para actualizaciones de archivos en tiempo real
document.addEventListener("DOMContentLoaded", () => {
    if (typeof VEHICULO_ID !== 'undefined') {
        const loc = window.location;
        const wsStart = loc.protocol === "https:" ? "wss://" : "ws://";
        const socket = new WebSocket(wsStart + loc.host + `/ws/vehiculo-archivos/${VEHICULO_ID}/`);

        socket.onmessage = function(event) {
            const data = JSON.parse(event.data);

            if (data.html) {
                document.getElementById("tabla-archivos").innerHTML = data.html;
            }
        };
